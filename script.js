...
// функция для вывода значения счетчика
function InformScore(score){
var inf = document.querySelector(".info").innerHTML="";
// создаем поле для отображения счетчика:
var gameinfo = document.createElement("p");
gameinfo.innerText = "Счет игры: " + score;
document.querySelector(".info").appendChild(gameinfo);
}
...
function startGame() {
  ...
  //   setTimeout(createBomb, 2000); //создаем бомбы
...
function move() {
    ...
    if (!isSnakeUnit(new_unit) && new_unit !== undefined) {
       ...
       var removed = snake.splice(0, 1)[0];
        ...
        // Проверяем, не попала ли змейка на бомбу. Если попала то убираем хвост еще на единицу
